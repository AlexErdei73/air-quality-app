...
export function getAllData(coord) {
  ...
  const weather = fetchJSON(weatherURL).then((data) => {
    return getWeatherFromJSON(data);
  });

  const pollution = fetchJSON(pollutionURL).then((data) => {
