...
async function getAPIData(request) {
  ...
  if (request == "weather") {
    let weatherURL =
      ...
      "&lon=" +
      lon +
      "&appid=0fb9ebda5842e2a12a43af55c70d2e3f";
    ...
    if (weather != undefined) {
      console.log(
        "The current weather description for your location is " + weather + "."
      );
    }
  ...
  //do an API request to get data about pollution at the user's location
    ...
    if (pm2_5 != undefined) {
      console.log(
        "The current pm2.5 reading for your location is " + pm2_5 + " μg/m3."
      );
      render(); //This line does the DOM rendering in the right time
    }
...
function fetchJSON(url) {
  const JSON = fetch(url, { mode: "cors" })
